...
# 六、动态目标栅格化并上色
在tools/visual_utils/open3d_vis_utils.py中原动态散点上色代码处修改，如下
```python
if point_colors is None:
        pts.colors = open3d.utility.Vector3dVector(np.ones((points.shape[0], 3)))
    else:
        ...
        color = np.ones_like(points) * [1, 1, 1] # 初始颜色为白色
        # confidence_threshold = 0.2 # 上色的置信度
        # color = add_color_ground(points,color) # 按z坐标给地面上色
        colored_points = []
        for i in range(len(ref_boxes)):
            box = ref_boxes[i]
            label = ref_labels[i]
            ...
            # if score <=confidence_threshold:
            #     continue
            color,bbox_meshs_car,bbox_meshs_person,colored_points = add_color(box,label,points,color,colored_points)
            if True: # 栅格化
                if bbox_meshs_car is not None: # 栅格化
                    vis.add_geometry(bbox_meshs_car)
                if bbox_meshs_person is not None:
                    vis.add_geometry(bbox_meshs_person)
        pts.colors = open3d.utility.Vector3dVector(color)
```
将栅格化代码改写成函数rasterization(points,label=5)，以及其余函数修改
```python
def color_points_in_bbox(points, bbox_center, bbox_size, bbox_rotation, color_in_bbox,color):
    rotated_points = rotate_points(points, bbox_center, bbox_rotation)
    bbox_min = bbox_center - bbox_size / 2
    ...
    inside_bbox = (np.all(rotated_points >= bbox_min, axis=1) & np.all(rotated_points <= bbox_max, axis=1))
    color[inside_bbox] = color_in_bbox
    return color, inside_bbox

def add_color(box,label,points,color,colored_points=None):
    bbox_center = np.array([box[0], box[1], box[2]])
    bbox_size = np.array([box[3], box[4], box[5]])
    bbox_rotation = box[6] 
    try:
        color_in_bbox = box_colormap2[label]
    except:
        color_in_bbox = np.array([1, 1, 1])
    # if label == 1:
    #     color_in_bbox = np.array([1, 0, 0])  # 白色
    # else:
    #     color_in_bbox = np.array([0, 1, 0])  # 绿色
    color, inside_bbox = color_points_in_bbox(points, bbox_center, bbox_size, bbox_rotation, color_in_bbox,color)
    bbox_meshs_car = None
    bbox_meshs_person = None
    if label == 0: # car
        points_car = []
        for i in range(len(inside_bbox)):
            is_inside = inside_bbox[i]
            if is_inside:
                points_car.append(points[i])
        a = 1
        if len(points_car) != 0:
            bbox_meshs_car = rasterization(points_car, label)
            colored_points = colored_points+points_car
    elif label == 1: # person
        points_person = []
        for i in range(len(inside_bbox)):
            is_inside = inside_bbox[i]
            if is_inside:
                points_person.append(points[i])
        if len(points_person) != 0:
            bbox_meshs_person = rasterization(points_person, label)
            colored_points = colored_points+points_person
    return color,bbox_meshs_car,bbox_meshs_person,colored_points

def rasterization(points,label=5):
    points = sorted(points, key=lambda p: (p[0], p[1], p[2])) # 排序
    try:
        color = box_colormap2[label]
    except:
        color = [1, 1, 1]
    count = 1
    for point in points:
...
```
可视化效果
![1](https://github.com/xingchenshanyao/VoxelNeXt/assets/116085226/1f4a4926-a311-4b49-91f8-f5e00665079b)

# 七、地面栅格化并上色
地面散点坐标由其他同学提供.pcd文件

修改tools/visual_utils/open3d_vis_utils.py，完整代码如下
```python
"""
Open3d visualization tool box
Written by Jihan YANG
...
def draw_scenes(points, gt_boxes=None, ref_boxes=None, ref_labels=None, ref_scores=None, point_colors=None, draw_origin=True, all_data =None,ground_points=None):
    if isinstance(points, torch.Tensor):
        points = points.cpu().numpy()
    if isinstance(gt_boxes, torch.Tensor):
        gt_boxes = gt_boxes.cpu().numpy()
    ...
    threshold,confidence_threshold=0.5,0.15
    # threshold = 0.5 为交并比阈值
    # confidence_threshold = 0.1 为置信度阈值
    ref_boxes, ref_scores = non_maximum_suppression(ref_boxes, ref_scores, threshold=threshold,confidence_threshold=confidence_threshold) # 非极大值抑制 # 20231225
    ...
    if point_colors is None:
        ...
        for i in range(len(ref_boxes)):
            ...
            # if score <=confidence_threshold:
            #     continue
            color,bbox_meshs_car,bbox_meshs_person,bbox_meshs_stop,colored_points = add_color(box,label,points,color,colored_points)
            if True: # 栅格化
                if bbox_meshs_car is not None: # 栅格化
                    vis.add_geometry(bbox_meshs_car)
                if bbox_meshs_person is not None:
                    vis.add_geometry(bbox_meshs_person)
                if bbox_meshs_stop is not None:
                    vis.add_geometry(bbox_meshs_stop)
        pts.colors = open3d.utility.Vector3dVector(color)
    ...
    if (ref_boxes is not None) and False: # 是否绘制检测框
        vis = draw_box(vis, ref_boxes, (0, 1, 0), ref_labels, ref_scores)
    if True: # 背景栅格化

        # 未上色的剩余点
        # least_points = [x for x in points if x not in colored_points] 
        points_tuples = [tuple(point) for point in points]
        colored_points_tuples = [tuple(point) for point in colored_points]

        least_points = np.array([point for point in points_tuples if point not in colored_points_tuples])
        
        bbox_meshs = rasterization(least_points)
        vis.add_geometry(bbox_meshs)
        # 地面点上色
        bbox_meshs_ground = rasterization(ground_points,10)
        vis.add_geometry(bbox_meshs_ground)
        # colored_points = colored_points+ground_points.tolist() # 计算量太大


    # # 获取观察视角控制对象
    # view_control = vis.get_view_control()

    # view_control.set_zoom(0.5)  # 缩放
    # # 设置新的观察视角
    # view_control.set_lookat([10, 10, 10])  # 观察点位置
    # view_control.set_front([-1, -1, -1])  # 观察方向
    # view_control.set_up([0, 0, 1])  # 上方向


    # ctr = vis.get_view_control()
    # ctr.set_lookat(np.array([0.0, 0.0, 55.0]))
    # ctr.set_up((-1, 0, 0))  # set the positive direction of the x-axis as the up direction
    # ctr.set_front((0, -1, 0))  # set the positive direction of the x-axis toward you
...
def draw_box(vis, gt_boxes, color=(0, 1, 0), ref_labels=None, score=None):
    ...
    for i in range(gt_boxes.shape[0]):
        ...
        if ref_labels is None:
            ...
            # line_set.paint_uniform_color(box_colormap[ref_labels[i]])
            try:
                line_set.paint_uniform_color(box_colormap2[ref_labels[i]])
            except:
                continue
...
def add_color(box,label,points,color,colored_points=None):
    ...
    if label == 0: # car
        ...
        for i in range(len(inside_bbox)):
            is_inside = inside_bbox[i]
            if is_inside:
                points_stop.append(points[i])
        if len(points_stop) != 0:
            bbox_meshs_stop = rasterization(points_stop, label)
            colored_points = colored_points+points_stop
        


    return color,bbox_meshs_car,bbox_meshs_person,bbox_meshs_stop,colored_points
...
def rasterization(points,label=11):
    points = sorted(points, key=lambda p: (p[0], p[1], p[2])) # 排序
    size = 0.3
    try:
        color = box_colormap2[label]
